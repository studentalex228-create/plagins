...
(function () {
    ...
    // Функция для запросов к Кинопоиску через стандартный fetch (как в rezkacomment)
    ...
    function showContentModal(kp_id, action, menuTitle, isTvShow, movieTitle, movieYear) {
        ...
        kpRequest(url, function(json) {
            ...
            Lampa.Modal.open({
                ...
                onBack: function() {
                    Lampa.Modal.close();
                    showExtraMenu(kp_id, movieTitle, movieYear, isTvShow);
                }
            });
        });
    }
    ...
    function startPlugin() {
        ...
        Lampa.Listener.follow("full", function (e) {
            ...
            // Прямое нахождение контейнеров (как в rezkacomment.js)
            ...
            btn.on('hover:enter', function () {
                ...
                var kpid = e.data.movie.kinopoisk_id || e.data.movie.kp_id || e.data.movie.id_kp;
                if (kpid) {
                    showExtraMenu(kpid, movieTitle, movieYear, isTvShow);
                } else {
                    // Умный поиск строго по чистому названию
                    var searchQuery = encodeURIComponent(movieTitle);
                    ...
                    .then(function(searchJson) {
                        ...
                        if (searchJson && searchJson.films && searchJson.films.length > 0) {
                            var bestMatch = searchJson.films[0].filmId;
                            // Сверка по годам для точного попадания новинок
                            if (movieYear) {
                                for (var f = 0; f < searchJson.films.length; f++) {
                                    var fYear = '' + searchJson.films[f].year;
                                    if (fYear && fYear.indexOf(movieYear) !== -1) {
                            ...
                            showExtraMenu(bestMatch, movieTitle, movieYear, isTvShow);
                        } else {
                            Lampa.Noty.show('Фильм не найден на Кинопоиске.');
                        }
                    })
            ...
            var btnContainer = $(".full-start-new__buttons");
            if (!btnContainer.length) btnContainer = $(".full-start__buttons");
            if (btnContainer.length) btnContainer.append(btn);
        });
    }
...
v class="full-review selector layer--visible type--line kp-review-card" style="min-width: 240px; margin-right: 15px;">' +
