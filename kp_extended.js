                ...
                if (kpid) {
                    ...
                    makeKpRequest(searchUrl, currentToken, function(searchJson) {
                        ...
                        if (items && items.length > 0) {
                            ...
                            // Если год не совпал, берем первый результат, чтобы не ломать меню
                            if (!bestMatch) bestMatch = items[0].filmId || items[0].kinopoiskId;

                            openMainMenu(bestMatch, currentToken);
                        } else {
                            Lampa.Noty.show('Фильм не найден на Кинопоиске.');
                        }
                    });
...
? page.find('.full-start-new__buttons') : page.find('.full-start__buttons');
            if (btnContainer.length) {
                btnContainer.append(btn);
            }
        });
