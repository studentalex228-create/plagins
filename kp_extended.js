(function () {
  ...
  function renderKpBlock(kpid, token, movie, results) {
    ...
    // Клик по карточке похожего фильма — ищем в TMDB и открываем карточку напрямую
    $block.find(".kp-similar-card").on("hover:enter", function () {
      var $card = $(this);
      var name = $card.data("name");
      var origName = $card.data("orig-name");
      var year = $card.data("year") || "";

      Lampa.Loading.start();
      findTmdbMovieAndOpenCard(name, origName, year, function (success) {
        Lampa.Loading.stop();
        if (!success) {
  ...
  /* ========================================================================= ПОИСК TMDB + ОТКРЫТИЕ КАРТОЧКИ (без зависаний) ========================================================================= */
  ...
  // Поиск в TMDB по имени + году, затем открытие карточки фильма.
  // Если что-то не получилось — возвращает false в callback.
  function findTmdbMovieAndOpenCard(name, origName, year, callback) {
    var queries = [];
    if (origName) queries.push(origName);
    if (name && name !== origName) queries.push(name);
    ...
    function tryNext(idx) {
      ...
      searchTmdbMulti(
        q,
        function (results) {
          var picked = pickBestTmdbResult(results, name, origName, year);
          if (picked) {
            // Проверяем, что фильм реально существует в TMDB (полный запрос)
            verifyTmdbAndOpen(picked, year, function (ok) {
              if (ok) callback(true);
              else tryNext(idx + 1);
            });
          } else {
  ...
  function searchTmdbMulti(query, onSuccess, onError) {
    var url =
    ...
    fetch(url)
      ...
      .then(function (data) {
        onSuccess(data && data.results ? data.results : []);
      })
      .catch(function () {
        onError();
      });
  ...
  // Выбираем лучший результат по имени/году
  function pickBestTmdbResult(results, name, origName, year) {
    if (!results || !results.length) return null;

    var normName = normalizeStr(name);
    var normOrig = normalizeStr(origName);
    ...
    function scoreOf(r) {
      ...
      if (normName && rTitle === normName) score += 100;
      else if (
        normName &&
        (rTitle.indexOf(normName) !== -1 || normName.indexOf(rTitle) !== -1)
      )
        score += 50;

      if (normOrig && rOrig === normOrig) score += 100;
      else if (
        normOrig &&
        (rOrig.indexOf(normOrig) !== -1 || normOrig.indexOf(rOrig) !== -1)
      )
        score += 50;

      if (mYear && rYear) {
        if (rYear === mYear) score += 80;
        else if (Math.abs(rYear - mYear) === 1) score += 30;
        else score -= 40;
      }

      // Бонус: фильмы с постером и высоким рейтингом обычно более релевантны
      if (r.poster_path) score += 5;
      if ((r.vote_count || 0) > 50) score += 5;
  ...
  // Запрашиваем полные данные фильма по ID и открываем карточку Lampa
  function verifyTmdbAndOpen(picked, year, callback) {
    ...
    var url =
      ...
      "?api_key=" +
      getTmdbKey() +
      "&language=ru-RU&append_to_response=credits,images";
    ...
    fetch(url)
      ...
      .then(function (data) {
        ...
        var movie = buildFullMovieObject(data, method);
        openCardViaActivity(movie);
        callback(true);
      })
      .catch(function () {
        callback(false);
  ...
  // Собираем полный объект movie в формате, который ждёт Lampa
  function buildFullMovieObject(data, method) {
    return {
      id: data.id,
  ...
  // Открываем карточку фильма через Lampa.Activity.push
  function openCardViaActivity(movie) {
    // Даём текущему контроллеру закрыть Select / списки
    setTimeout(function () {
      Lampa.Activity.push({
        url: "",
        component: "full",
