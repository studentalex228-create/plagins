...
(function () {
    ...
    function startPlugin() {
        ...
        Lampa.Listener.follow("full", function (e) {
            ...
            function loadImages(kp_id, type, title, containerClass) {
                apiRequest(`v2.2/films/${kp_id}/images?type=${type}&page=1`, (json) => {
                    if (json?.items?.length) {
                        createSection(containerClass, title);
                        const itemsBlock = page.find(`.${containerClass}-items`);
                        json.items.forEach(img => {
                            const item = $(`<div class="selector layer--visible kp-image-card" style="margin-right: 15px; background: url('${img.previewUrl}') center/cover no-repeat;"></div>`);
                            item.on('hover:enter', () => {
                                Lampa.Modal.open({ 
                                    title: title, 
            ...
            // Ищем ID в карточке или через поиск (без сторонних прокси!)
            const kpid = e.data.movie.kinopoisk_id || e.data.movie.kp_id || e.data.movie.id_kp;
            if (kpid) {
                startFetching(kpid);
            } else {
                const query = encodeURIComponent(movieTitle + (movieYear ? ` ${movieYear}` : ''));
                apiRequest(`v2.1/films/search-by-keyword?keyword=${query}`, (searchJson) => {
                    if (searchJson?.films?.length) startFetching(searchJson.films[0].filmId);
                    else Lampa.Noty.show('❌ Не найдено на Кинопоиске');
    ...
    else if (window.LampaListener) window.LampaListener.follow("app", (e) => { if (e.type === "ready") startPlugin(); });
