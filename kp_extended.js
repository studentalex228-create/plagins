...
(function () {
            ...
            try {
                ...
                $(".button--kp-main-plus").on("hover:enter", function () {
                    try {
                        ...
                        if (kpid) {
                            Lampa.Loading.stop();
                            openMenu(kpid, movieTitle, movieYear, isTvShow);
                        } else {
                            var searchQuery = encodeURIComponent(movieTitle);
                            var network = new Lampa.Reguest();
                            
                            // Используем родной Lampa.Reguest() для 100% обхода CORS на cf.lampa.mx
                            network.silent('https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=' + searchQuery, function(searchJson) {
                                ...
                                if (items && items.length > 0) {
                                    ...
                                    openMenu(bestMatch, movieTitle, movieYear, isTvShow);
                                } else {
                        ...
                        Lampa.Loading.stop();
                        Lampa.Noty.show('Ошибка запуска: ' + ex.message);
                    }
                });
            } catch(ex) {
                Lampa.Noty.show('Ошибка плагина: ' + ex.message);
            }
        });
    }

    // Безопасная функция внутренних запросов через ядро Lampa
    function kpRequest(endpoint, cb) {
        var token = Lampa.Storage.get('kp_unofficial_token', '');
        var network = new Lampa.Reguest();
    ...
    // Главное меню выбора материалов
    function openMenu(kp_id, movieTitle, movieYear, isTvShow) {
        try {
            var items = [
                { title: '💡 Интересные факты', action: 'facts' },
                { title: '🚫 Киноляпы и ошибки', action: 'bloopers' },
            ...
            Lampa.Select.show({
                ...
                onSelect: function (item) {
                    try {
                                ...
                                if (json && json.items && json.items.length > 0) {
                                    ...
                                    Lampa.Select.show({
                                        ...
                                        onSelect: function(selectedSim) {
                                            ...
                                            var uToken = Lampa.Storage.get('kp_unofficial_token', '');
                                            var netDetails = new Lampa.Reguest();
                                            
                                            netDetails.silent('https://kinopoiskapiunofficial.tech/api/v2.2/films/' + selectedSim.id, function(details) {
                                                Lampa.Loading.stop();
                                                if (details && details.imdbId) {
                                                    var method = 'movie';
                                                    if (details.type && (details.type.indexOf('SERIES') !== -1 || details.type.indexOf('SHOW') !== -1)) {
                                                ...
                                                headers: { 'X-API-KEY': uToken, 'Content-Type': 'application/json' }
                                            });
                                        },
                                        onBack: function() { openMenu(kp_id, movieTitle, movieYear, isTvShow); }
                                    });
                                } else {
                                    Lampa.Noty.show('Похожих фильмов не найдено.');
                                    Lampa.Controller.toggle('content');
                        ...
                        Lampa.Noty.show('Ошибка обработки: ' + ex.message);
                    }
                },
                onBack: function () {
                    Lampa.Controller.toggle('content');
            ...
            Lampa.Noty.show('Ошибка меню: ' + ex.message);
        }
    }
        ...
        try {
            ...
            kpRequest(url, function(json) {
                ...
                Lampa.Modal.open({
                    ...
                    onBack: function() {
                        Lampa.Modal.close();
                        openMenu(kp_id, movieTitle, movieYear, isTvShow);
                    }
                });
            });
        } catch(ex) {
            Lampa.Loading.stop();
            Lampa.Noty.show('Ошибка отображения: ' + ex.message);
        }
    }
    ...
    try {
        ...
        console.error('KP Extension init error', err);
    }
})();
