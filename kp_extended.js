    ...
    function startPlugin() {
        ...
        Lampa.Listener.follow('full', function (e) {
            ...
            btn.on('hover:enter', function () {
                ...
                // Логика сопоставления ID фильма
                var kpid = e.data.movie.kinopoisk_id || e.data.movie.kp_id || e.data.movie.id_kp;
                if (kpid) {
                    openExtraMenu(kpid);
                } else {
                    // ИСПРАВЛЕНО: Ищем по чистому названию (избегаем 0 результатов из-за разницы баз)
                    ...
                    network.silent('https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=' + searchQuery, function(searchJson) {
                        if (searchJson && searchJson.films && searchJson.films.length > 0) {
                            ...
                            // ИСПРАВЛЕНО: Умная сверка годов (устраняет проблему перепутанных фильмов)
                            if (movieYear) {
...
opoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=' + searchQuery, function(searchJson) {
                        if (searchJson && searchJson.films && searchJson.films.length > 0) {
